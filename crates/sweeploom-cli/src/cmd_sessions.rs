//! `sweeploom sessions` — live grouping plus optional RAM/CPU plan.
...
use sweeploom_session::{AttributionRoots, plan_free_ram, plan_reduce_cpu, sessions_from_snapshot};

use crate::bytes::format_bytes;

pub fn run(args: impl Iterator<Item = String>) {
    let (free_ram, reduce_cpu) = parse(args);
    ...
    let planned = planned_ids(&sessions, free_ram, reduce_cpu);
    ...
    if free_ram.is_some() || reduce_cpu.is_some() {
        println!(
...
fn parse(args: impl Iterator<Item = String>) -> (Option<f64>, Option<f32>) {
